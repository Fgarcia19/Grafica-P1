    ...
    if (hay_interseccion and pObjeto->es_luz){
        ...
        for (auto l: luces)
        {
            ...
            if (!esta_sombra) {
                ...
                if (factor_difuso > 0) {
                    luz_difusa = luz_difusa + (l->color * pObjeto->kd * factor_difuso) / luces.size();
                ...
                if (factor_especular > 0) {
                    luz_especular =
                            luz_especular + (l->color * pObjeto->ks * pow(factor_especular, pObjeto->n)) / luces.size();
                }
...
void Camara::renderizar(vector<Objeto*> &objetos, vector<Luz*> &luces, int num) {
    ...
    dis_img.render((*pImg));
    dis_img.paint();
    string nombre_archivo = "img" + to_string(num) + ".bmp";
